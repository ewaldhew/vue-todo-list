...
export default new Vuex.Store({
    ...
    actions: {
        loadTodoCollection({ commit }) {
            return api.getData(api.loadLists())
                .then((todoCollection) => {
        ...
        loadTodos({ commit }, id) {
            return api.getData(api.loadTodoList(id))
                .then((todoList) => {
        ...
        toggleTodo({ commit }, id) {
            ...
            commit('TOGGLE_TODO')
            return api.editTodo(id, this.state.todo)
                .then(() => {
                    return { ok: true }
                })
        },
        ...
        createTodo({ commit }, { id, name }) {
            return api.createTodo(id, name)
                .then(() => {
                    return { ok: true }
                })
        },
        createTodoList({ commit }, name) {
            return api.createTodoList(name)
                .then(() => {
                    return { ok: true }
                })
        },
